...
import urllib.parse, urllib.request, urllib.error, http.client, time, cgi, html.parser
import io, base64, hmac, hashlib, datetime, re, random
...
def normalize_uri(uri):
    ...
    parts = urllib.parse.urlparse(uri) # already lower-cases scheme
    ...
    parts[1] = netloc
    parts[2] = path
    return 'uri', urllib.parse.urlunparse(parts)
...
class OpenIDParser(html.parser.HTMLParser):
    def __init__(self):
...
def resolve_xri(xri, proxy='xri.net'):
    ...
    xri = urllib.parse.quote(xri, safe='=@*!+()')
...
def unbtwoc(B):
    return pickle.loads(b('\x80\x02\x8a')+bytes_from_ints([len(B)])+B[::-1]+(b'.'))
...
"""
dh_prime = int("".join(dh_prime.split()), 16)
...
def associate(services, url):
    ...
    if is_compat_1x(services):
        ...
        del data['openid.ns']
    res = urllib.request.urlopen(url, b(urllib.parse.urlencode(data)))
    if res.getcode() != 200:
...
def authenticate(session, response):
    ...
    if session['assoc_handle'] != response['openid.assoc_handle']:
    ...
    signed = response['openid.signed'].split(',')
    query = []
    for name in signed:
        value = response['openid.'+name]
        value = '%s:%s\n' % (name, value)
    ...
    transmitted_sig = base64.decodestring(b(response['openid.sig']))
